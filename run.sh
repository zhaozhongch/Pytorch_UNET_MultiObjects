...
exec docker run \
     ...
     -v /home/zhaozhong/dataset/carvana:/opt/dataset \
     --ipc=host \
     zhch5450/u_net:1 /bin/bash -c "cd /mnt/shared"
exec docker pytorch_test_container #Use this to show what happened in docker	  
exit $
#compile elastic fusion
#"cd /mnt/shared && cd ElasticFusion && cd Core && mkdir build && cd build && cmake ../src && make -j8 && cd ../GPUTest && mkdir build && cd build && cmake ../src && make -j8 && cd ../GUI && mkdir build && cd build && cmake ../src make -j8"
#-v /home/zhaozhong/dataset/dyson_lab:/opt/dataset \